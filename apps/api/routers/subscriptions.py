...
def list_plans(session: Session = Depends( get_session_depends)):
    stmt = select(Plan)
    return session.exec(stmt).all()

@router.get("/me", response_model=List[UserSubscription])
def my_subscriptions(session: Session = Depends( get_session_depends), user = Depends(get_current_user)):
    stmt = select(UserSubscription).where(UserSubscription.user_id == user.id)
    return session.exec(stmt).all()

@router.post("/demo/activate", status_code=201)
def demo_activate(plan_key: str, session: Session = Depends( get_session_depends), user = Depends(get_current_user)):
...
def cancel(subscription_id: int, session: Session = Depends( get_session_depends), user = Depends(get_current_user)):
...
def list_audit(session: Session = Depends( get_session_depends), admin = Depends(get_admin_user)):
